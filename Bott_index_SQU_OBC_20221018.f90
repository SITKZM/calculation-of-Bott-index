...
program Bott_index_SQU_OBC
    ...
    real, parameter :: mu = -0.0, h_z = 0.5, V = 0.5, Delta = 0.2, pi = 4*atan(1.)
    !variable
    integer :: INFO
    ...
    contains
        ...
        subroutine get_Bott_index(B, eigm)
            ...
            do i = 1, N
                !particle
                !spin up
                X(2*i-1, 2*i-1) = exp(cmplx(0, 2*pi*real(modulo(i-1, N_x))/(N_x-1)))
                Y(2*i-1, 2*i-1) = exp(cmplx(0, 2*pi*real(floor(real(i-1)/N_x))/(N_y-1)))
                !spin down
                X(2*i, 2*i) = exp(cmplx(0, 2*pi*real(modulo(i-1, N_x))/(N_x-1)))
                Y(2*i, 2*i) = exp(cmplx(0, 2*pi*real(floor(real(i-1)/N_x))/(N_y-1)))
                !hole
                !spin up
                X(2*i-1+2*N, 2*i-1+2*N) = exp(cmplx(0, 2*pi*real(modulo(i-1, N_x))/(N_x-1)))
                Y(2*i-1+2*N, 2*i-1+2*N) = exp(cmplx(0, 2*pi*real(floor(real(i-1)/N_x))/(N_y-1)))
                !spin down
                X(2*i+2*N, 2*i+2*N) = exp(cmplx(0, 2*pi*real(modulo(i-1, N_x))/(N_x-1)))
                Y(2*i+2*N, 2*i+2*N) = exp(cmplx(0, 2*pi*real(floor(real(i-1)/N_x))/(N_y-1)))
            end do
