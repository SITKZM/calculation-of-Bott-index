...
def get_Bott_index(N_x, N_y, eigm):
    ...
    for i in range(N):
        #particle
        #spin up
        X[2*i, 2*i] = (i % N_x)/(N_x - 1)
        Y[2*i, 2*i] = (np.floor(i/N_x))/(N_y - 1)
        #spin down
        X[2*i+1, 2*i+1] = (i % N_x)/(N_x - 1)
        Y[2*i+1, 2*i+1] = (np.floor(i/N_x))/(N_y - 1)
        #hole
        #spin up
        X[2*i+2*N, 2*i+2*N] = (i % N_x)/(N_x - 1)
        Y[2*i+2*N, 2*i+2*N] = (np.floor(i/N_x))/(N_y - 1)
        #spin down
        X[2*i+1+2*N, 2*i+1+2*N] = (i % N_x)/(N_x - 1)
        Y[2*i+1+2*N, 2*i+1+2*N] = (np.floor(i/N_x))/(N_y - 1)
